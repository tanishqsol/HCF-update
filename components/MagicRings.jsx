...
vec3 paletteColor(float mixAmount) {
  if (mixAmount < 0.33) {
    return mix(uColor, uColorTwo, smoothstep(0.0, 0.33, mixAmount));
  }

  if (mixAmount < 0.66) {
    return mix(uColorTwo, uColorThree, smoothstep(0.33, 0.66, mixAmount));
  }
...
void main() {
  ...
  for (int i = 0; i < 10; i++) {
    ...
    float mixAmount = fi / rcf;
    vec3 rc = paletteColor(mixAmount);
